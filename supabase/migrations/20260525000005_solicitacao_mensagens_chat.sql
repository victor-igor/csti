...
-- SELECT: Permite ler se o usuário for admin, o próprio cliente dono da solicitação,
-- ou um prestador que está autorizado a ver a solicitação
CREATE POLICY "mensagens_select_policy" ON public.mensagens_solicitacao
  FOR SELECT USING (
    ...
    -- Um prestador que esteja aprovado
    (
...
-- INSERT: Permite inserir se o usuário estiver aprovado e for participante do fluxo
CREATE POLICY "mensagens_insert_policy" ON public.mensagens_solicitacao
  FOR INSERT WITH CHECK (
    auth.uid() = usuario_id AND
    public.check_user_approved(auth.uid())
  );
